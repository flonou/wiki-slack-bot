...
#           @@logger.debug("Id : #{asset["id"]} \tName : #{asset["1"]}\tType : #{asset["itemtype"]}")
        ...
        if(messageTs.nil?)
          webclient.chat_postMessage(channel: channel, as_user: true, text: message, blocks: blockActions)
        else
          webclient.chat_update(channel: channel, as_user: true, text: message, ts: messageTs, blocks: blockActions)
        end
      rescue => e
...
#         @@logger.debug(asset)
      ...
      slackResp = webclient.chat_update(channel: channel, text: message, as_user: true, ts: messageId, blocks: blockActions)
